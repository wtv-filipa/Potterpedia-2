...
import { updateSpell1 } from "../../../store/spells/actions";
...
const UpdateSpellCard = ({ id, effect = "", spell = "", type = "" }) => {
  const [initialValues, setInitialValues] = useState({ effect, spell, type });
  const dispatch = useDispatch();

  const onConfirm = (effect, spell, type) => {
    dispatch(updateSpell1(id, effect, spell, type));
  };
  return (
    <div id="spells" className="col-12" key={id}>
      <Formik
        validateOnBlur
...
export default UpdateSpellCard;
